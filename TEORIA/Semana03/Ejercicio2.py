...
def cuantas_peliculas(cursor):
    ...
    cursor.execute("SELECT * FROM movie_ratings.usuario_califica_peliculas WHERE nombre_usuario='Matt';")#Se hace la consulta a Mysql, con el comando para buscar todas las calificaciones que hizo Matt
...
try:    #Si todo sale bien
    ...
    if conexion.is_connected():     #Condicional para verificar si nos conectamos a la bd.
        ...
        print("-> Matt califico: ",cuantas_peliculas(cursor)," Peliculas.")   #Se llama a la funcion que calculara el numero de peliculas de Matt.
