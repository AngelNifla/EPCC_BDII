...
#3)¿Qué película fue la menor calificada por los usuarios?, ¿cuántos usuarios la calificaron?
...
def pelicula_menor(cursor): #funcion que nos devolvera la pelicula mas califiacada.
    ...
    menor = 26   #variable para el numero con menor
    cont = 0    #variable contador de califiaciones
    peli_menor = NULL   #variable para almacenar el numero de tupla de peli con menor califiacaiones
    for pelicula in name_peliculas:    #bucle para comparar cada peli pelicula=[(una_peli,),()...]
        ...
        if cont < menor:    #al finalizar el bucle de calificaciones, si el contador es menor que el contador de la comparacion anterior
            menor = cont    #el contador actual se guarda en la variable menor,
            peli_menor = pelicula[0] #se guarda el numero de la tupla de la peli que gano mas calificaciones que la anterior
        cont = 0    #y volvemos al contador a 0.
    print("-> La pelicula menos calificada por los usuarios es la pelicula de id: ",peli_menor)#Se imprime la pelicula con mas califiaciones.
    
    return menor#y se retorna el nombre de la pelicula con menor calificaiones.
...
try:    #Si todo sale bien
    ...
    if conexion.is_connected():     #Condicional para verificar si nos conectamos a la bd.
        ...
        print("-> El numero de usuarios que calificaron esta pelicula es: ",pelicula_menor(cursor), " usuarios")   #Se imprimira el valor que devuelva la funcion pelicula_menor().
